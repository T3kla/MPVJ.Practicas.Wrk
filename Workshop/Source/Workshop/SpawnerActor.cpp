...
void ASpawnerActor::Tick(float DeltaTime)
{
    ...
    FColor col = ClassToSpawn_IsSelected ? ClassToSpawn_SelectedColor : ClassToSpawn_UnSelectedColor;

    DrawDebugDirectionalArrow(GetWorld(), startPos, endPos, 300.f, col, false, (-1.0f), (uint8)'\000', 2.0f);
...
inline void ASpawnerActor::SpawnStuff()
{
    ...
    ClassToSpawn_Instance = GetWorld()->SpawnActor<AActor>(ClassToSpawn, spawnedTransform);
}
...
bool ASpawnerActor::OnActorSpawnChanged() const
{
    return true;
}
